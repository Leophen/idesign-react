...
const TimeInput: React.FC<TimeInputProps> = (props) => {
  ...
  const inputChangeHour = (val: string | number) => {
  ...
  const inputChangeMinute = (val: string | number) => {
    val = _.padStart(String(val), 2, '0');
  ...
  const inputChangeSecond = (val: string | number) => {
    val = _.padStart(String(val), 2, '0');
  ...
  const timeColon = (
    <div className={classNames('i-time-colon', disabled && 'i-time-colon__disabled')}>:</div>
  );

  return (
    <Input placeholder="" readonly={!disabled} disabled={disabled} suffixIcon="Clock">
      {show.hour && (
        <>
          <Input
            size="small"
