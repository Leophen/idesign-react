...
const PreviewDialog: React.FC<PreviewDialogProps> = (props) => {
  const { visible = false, closeOnEsc = true, image, x = 0, y = 0, onClose = () => {} } = props;
  ...
  return (
    <div className="i-preview-dialog-wrapper">
      <Transition timeout={200} in={visible} animation="fade-in" key="i-preview-dialog__mask">
        <div
          className="i-preview-dialog__mask"
          onMouseDown={closePreviewDialog}
          onScroll={() => {
            return;
          }}
        >
          ...
          <div className="i-preview-dialog__handle" onMouseDown={clickHandleBar}>
            ...
            <section>
              <Icon
                ...
                size={24}
                color="#fff"
                onClick={() => clickHandle('reRotate')}
              />
              <Icon
                name="RefreshRight"
                size={24}
                color="#fff"
                onClick={() => clickHandle('rotate')}
              />
            </section>
          </div>
          <img
